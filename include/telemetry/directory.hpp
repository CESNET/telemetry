...
public:
	...
	/**
	 * @brief Add a symbolic link to the directory.
	 *
	 * This function creates a symbolic link with the specified @p name that points to the
	 * target node specified by @p target. If a node with the same name already exists, or if the
	 * symlink is attempting to point to itself, an exception will be thrown.
	 *
	 ...
	 *
	 * @throw TelemetryException If a symbolic link or file with the same name already exists in the
	 * directory, or if the symlink attempts to point to itself.
	 */
